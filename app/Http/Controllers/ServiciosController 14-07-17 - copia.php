...
class ServiciosController extends Controller
{
    ...
    public function AddActividad($elemento)
    {
        ...
        if ($elemento["tipo"] == 'Hotel') {
            ...
            // $DetalleItinerario = array(0 => array('TipoDetalleItinerarioActividadId' => 1, 'DatosDetalle' => "Detalle_Itinerario_Hotel_Ejemplo_Cesar"));
            ...
            // $Descripcion                                            = array(0 => array('TipoDetallesActividadId' => 1, 'DataActividad' => substr($elemento["datos"]->Descripcion, 0, 100)));
            $Descripcion                             = array();
            ...
            $DetalleItinerario = array(0 => array('TipoDetalleItinerarioActividadId' => 1, 'DatosDetalle' => $elemento["datos"]->Ruta));
            ...
            $Descripcion       = array(0 => array('TipoDetallesActividadId' => 1, 'DataActividad' => 'Tour Por :' . $elemento["datos"]->Ruta));
            ...
            $LongitudFinal     = $elemento["datos"]->Longitud;
            $Descripcion       = array(0 => array('TipoDetallesActividadId' => 1, 'DataActividad' => substr($elemento["datos"]->Descripcion, 0, 100)));
            $DetalleItinerario = array(0 => array('TipoDetalleItinerarioActividadId' => 1, 'DatosDetalle' => 'Actividad :' . $elemento["packagerooms"]->ActividadTarifas->ActividadTarifas->Nombre));
        ...
        $headers = array('Token' => '2a64025c-6a49-484a-99fc-fe285f0ae7a9',
            'Language'               => 'ES-MX',
            'Currency'               => 'MXN',
            'content-type'           => 'application/json');
        $body[] = array(
            'ItinerarioActividadesPadreId' => 0,
            'ItinerarioId'                 => $SessionItinerario->data->ItinerarioId,
            'FechaInicio'                  => $FechaInicio,
            'FechaFinal'                   => $FechaFinal,
            'DetalleItinerarioActividad'   => $DetalleItinerario,
            'ProveedorId'                  => 6,
            'TipoActividadId'              => $TipoActividadId,
            'CodigoProducto'               => $elemento["id"],
            'CodigoPaquete'                => $elemento["idPaquete"],
            'Calificacion'                 => $Calificacion,
            'Nom_producto'                 => $elemento["datos"]->Nombre,
            'Lugar'                        => $Lugar,
            'Latitud'                      => array(
                0 => array(
                    'CoordenadaTipoId' => 1,
                    'Latitudes'        => $LatitudInicio,
                ),
                1 => array(
                    'CoordenadaTipoId' => 2,
                    'Latitudes'        => $LatitudFinal,
                ),
            ),
            'Longitud'                     => array(
                0 => array(
                    'TipoCoordenadaId' => 1,
                    'Longitudes'       => $LongitudInicio,
                ),
                1 => array(
                    'TipoCoordenadaId' => 2,
                    'Longitudes'       => $LongitudFinal,
                ),
            ),
            'DetallesActividad'            => $Descripcion,
            'Precios'                      => $elemento["packagerooms"]->PricePackage,
        );

        // dd(json_encode($body));
        try {
            $Client   = new Client();
            $request  = new Requestt('POST', 'http://192.168.1.86:5050/Proyecto/Api/v1/ActividadesItinerarios', $headers, $actividad);
...
###################################VOUCHER  ########################################
                    #crearVaucher($venta, $indice, $idSession)
                    //  $VoucherDatos = array();
                    // array_push($voucher, $voucher);
    ...
    public function PreAutorizar(Request $request)
    {
        ...
        $body = array(
            'IdParteRoleProveedor'         => 1,
            'IdTipoTransaccionFinanciera'  => 3,
            'Monto'                        => 1, //por pruebas se maneja en 1
            'UUIDCliente'                  => $UUIDCliente, //debe ser dinamico
            'DetalleTransaccionFinanciera' => array(
                0 => array(
                    'IdTipoDetalleTransaccionFinanciera' => 1,
                    'Data'                               => 'RESER_' . $UUIDCliente, //debe ser dinamico
                ),
                1 => array(
                    'IdTipoDetalleTransaccionFinanciera' => 7,
                    'Data'                               => $this->ccnum,
                ),
                2 => array(
                    'IdTipoDetalleTransaccionFinanciera' => 8,
                    'Data'                               => $this->expiry,
                ),
                3 => array(
                    'IdTipoDetalleTransaccionFinanciera' => 9,
                    'Data'                               => $this->cvc,
                ),
            ),
        );
    ...
    {
        // dd($request->input('nombre_pasajero')[0]);
...
########################### Completar Datos en servicio Otisa !###############################
    ...
    public function finalizar(Request $request)
    {
        ...
        $this->DatosdelCarrito = session("carrito");
        $datos                 = array('carrito' => $this->DatosdelCarrito,
            'monto_total'                            => $this->total,
            'errores'                                => $error,
        );
    ...
    {
        // Eliminar_Sesiones(); resumenPreReserva($idSession)
        obtenerTotalCarrito();
        if ((session()->has('carrito_temp')) && (empty(session("carrito_temp")))) {
...
################################ agregando la actividad al itinerario ####################
            ...
            case 'Tour':
                try {
                    ...
                    $idPaquete = $request->input("idPaquete");
// //dd();     ## quitar el pruducto si existe
